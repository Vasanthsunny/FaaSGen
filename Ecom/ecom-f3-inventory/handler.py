...
def record_metrics(job_id, arrival, start, end, cpu_total):
    ...
    r.set(f"job:{job_id}:ecom-f3-inventory", json.dumps(metrics))
...
async def inventory(request: Request):
    ...
    # Simulate inventory DB latency
    time.sleep(0.7)

    stock_available = random.choice([True, True, True, False])

    r.set(f"job:{job_id}:inventory",
          json.dumps({"available": stock_available}))
    ...
    return {"job_id": job_id, "inventory_checked": stock_available}
