...
bbox_head = torch.nn.Sequential(
    ...
    torch.nn.Flatten(),
    torch.nn.Linear(1280, 4)  # x, y, w, h
)
bbox_head.eval()
...
@app.post("/")
async def bbox(request: Request):
    global peak_memory
    ...
    # ---- Forward pass ----
    with torch.no_grad():
        bbox = bbox_head(torch_features)

    # ---- Serialize bbox ----
    buf = io.BytesIO()
    np.save(buf, bbox.numpy())
    ...
    # ---- Store output for F6 ----
    r.set(f"job:{job_id}:bbox", encoded)
    ...
    return {"job_id": job_id, "status": "fd-f4-done"}
