...
# ---- Landmark head (5 points → 10 values) ----
landmark_head = torch.nn.Sequential(
...
@app.post("/")
async def landmark(request: Request):
    global peak_memory
    ...
    # ---- Forward pass ----
    with torch.no_grad():
        landmarks = landmark_head(torch_features)
    peak_memory = max(peak_memory, process.memory_info().rss)
    # ---- Serialize output ----
    buf = io.BytesIO()
    np.save(buf, landmarks.numpy())
    ...
    r.set(f"job:{job_id}:landmarks", encoded)
    ...
    return {"job_id": job_id, "status": "fd-f5-done"}
